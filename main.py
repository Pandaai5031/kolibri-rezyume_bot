...
# Telegram Bot Token
BOT_TOKEN = "8895389668:AAHKkjNuIlpjylzQPVBQgCmqegHQCA2hbxU"

bot = Bot(token=BOT_TOKEN)
dp = Dispatcher(storage=MemoryStorage())

# FSM holatlari (Savollar ketma-ketligi)
...
    city = State()
    summary = State()
    experience = State()      # Oldingi ish joylari
    education = State()       # O'qigan joylari
    skills = State()          # Ko'nikmalar
    languages = State()       # Biladigan tillari
    certificates = State()    # Sertifikatlar va kurslar
    bad_habits = State()      # Zararli odatlar
    driving_license = State() # Haydovchilik guvohnomasi
    expected_salary = State() # Kutilayotgan maosh
...
    # Bo'limlarni qo'shuvchi yordamchi funksiya
    def add_section(title, content):
        if content:
            story.append(Paragraph(title.upper(), heading_style))
            # Qatordan-qatorga ko'chirish uchun \n larni <br/> ga almashtiramiz
...
    # Qo'shimcha ma'lumotlar jadvali (Zararli odatlar va Haydovchilik guvohnomasi)
...
    asyncio.run(main()) botimni qanday ishga tushiraman 
